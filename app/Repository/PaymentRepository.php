...
class PaymentRepository implements PaymentRepositoryInterface
{
    ...
    public function update($request)
    {
        ...
        try {
            ...
            // حفظ البيانات في جدول الصندوق
            $fund_accounts = fund_accounts::where('payment_id',$request->id)->first();
            $fund_accounts->date = date('Y-m-d');
            $fund_accounts->payment_id = $payment_students->id;
            $fund_accounts->Debit = 0.00;
            $fund_accounts->credit = $request->Debit;
    ...
    $payment_students = $request->id;

    try {
    DB::beginTransaction();
      // حذف سجلات StudentAccount المرتبطة بـ Fee_invoice
      StudentAccount::where('payment_id', $payment_students)->delete();
        fund_accounts::where('payment_id', $payment_students)->delete();
